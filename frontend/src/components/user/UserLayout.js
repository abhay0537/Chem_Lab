    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-5 border-b border-slate-200 dark:border-slate-700">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-primary-600 rounded-xl flex items-center justify-center text-xl">⚗️</div>
          <div>
      ...
      <nav className="flex-1 px-3 py-3 space-y-0.5">
        ...
        {isAdmin && (
          <button onClick={() => navigate('/admin')}
            ...
            <span>🔧</span><span>Switch to Admin</span>
          </button>
        )}
      </nav>
    ...
    <div className="flex h-screen bg-slate-50 dark:bg-slate-900 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="lg:hidden flex items-center justify-between px-4 py-3 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
          <button onClick={() => setSidebarOpen(true)} className="text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 text-xl">☰</button>
          <div className="flex items-center gap-2">
            <span className="text-xl">⚗️</span>
            <span className="font-bold text-slate-900 dark:text-white">ChemLab</span>
          </div>
